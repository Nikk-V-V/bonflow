...
export default function (set, get) {
    return {
        ...
        move: (moveIndex) => {
            set((state) => {
                const { getCurrentPlayer, ratRaceCells, fastTrackCells, cellsScale } = state;
                ...
                const movementPositions = Array.from({ length: moveIndex }, (_, i) => (currentCellIndex + (i + 1)) % totalCells).map(index => cells[index].position);

                const movementPath = movementPositions.map((position) => {
                    const cellPosition = new THREE.Vector3(position.x, position.y, position.z);
                    return calculatePlayerPosition(cellPosition, circleType, cellsScale);
                });
        ...
        processMove: () => {
            const { ratRaceCells, fastTrackCells, getCurrentPlayer, handleCard, nextTurn } = get();
            const currentPlayer = getCurrentPlayer();
        ...
        initPlayers: () => {
            ...
            const cellPosition = new THREE.Vector3(initialCell.position.x, initialCell.position.y, initialCell.position.z);
