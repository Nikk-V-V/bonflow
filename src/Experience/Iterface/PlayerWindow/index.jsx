            ...
            if (isDragging.current) {
                ...
                requestAnimationFrame(() => {
                    ...
                    if (resizeDirection.current.includes('e')) {
                        newWidth = Math.max(MIN_WIDTH, Math.min(MAX_WIDTH, e.clientX - position.x))
                    } else if (resizeDirection.current.includes('w')) {
                        const deltaX = position.x - e.clientX
                        newWidth = Math.max(MIN_WIDTH, Math.min(MAX_WIDTH, size.width + deltaX))
                        newX = e.clientX
                    }

                    if (resizeDirection.current.includes('s')) {
                        newHeight = Math.max(MIN_HEIGHT, Math.min(MAX_HEIGHT, e.clientY - position.y))
                    } else if (resizeDirection.current.includes('n')) {
                        const deltaY = position.y - e.clientY
                        newHeight = Math.max(MIN_HEIGHT, Math.min(MAX_HEIGHT, size.height + deltaY))
                        newY = e.clientY
                    }
        ...
        >
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <Avatar className="w-8 h-8">
                            <AvatarImage src={player.model.avatar} alt={player.name} />
            ...
            {!isCollapsed && (
                <>
                    ...
                    <ScrollArea className={`h-[${size.height - 120}px] max-h-[70vh] overflow-auto`}>
                        <div className="p-4">
