    ...
    const togglePlayerWindow = (player) => {
        setOpenWindows(prevWindows => {
            ...
            if (existingWindow) {
                ...
                const newWindow = {
                    ...player,
                    position: { x: (window.innerWidth - (300 + prevWindows.length * 100)), y: (0) }
                }
    ...
    return (
        <div className="fixed top-0 z-10 flex text-white p-6">
                            ...
                            <TooltipTrigger asChild>
                                <button
                                    className={`relative rounded-full ${index === turnPlayerIndex  ? 'ring-2 ring-purple-500' : ''} ${
                                        openWindows.some(w => w.id === player.id) ? 'bg-purple-500' : ''
                                    }`}
                                    onClick={() => togglePlayerWindow(player)}
                                >
            ...
            <div className="flex-grow static">
                <AnimatePresence>
                    {openWindows && openWindows.map((player) => (
                        <PlayerWindow
                            key={player.id}
                            player={player}
